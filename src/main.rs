...
fn main() {
  ...
  let plan_total_hour = portal_plan.total_hour().hour() + gw_cloud_plan.total_hour().hour() + service_plan.total_hour().hour();
  println!("プロジェクト間分割不可能時間 (プロジェクト間分割不可能日に追加): {:.2} 時間", total_hour - plan_total_hour);
}
